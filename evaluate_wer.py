...
def evaluate(dataset_path, skip_whisper=False):
    ...
    # Delay whisper loading until it's actually needed
    model = None
    ...
    for sample in dataset:
        ...
        # If we dont have a pre-computed hypothesis, try to transcribe
        if not hypothesis:
            if skip_whisper:
                print(f"Report ID: {report_id} | Skipped (No pre-computed hypothesis and skip_whisper=True)")
                print("-" * 50)
                continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Word Error Rate (WER)")
    parser.add_argument("--dataset", type=str, default="mock_wer_dataset.json", help="Path to json dataset")
    parser.add_argument("--skip-whisper", action="store_true", help="Do not run Whisper, rely only on pre-computed 'hypothesis' in JSON")
    args = parser.parse_args()

    evaluate(args.dataset, args.skip_whisper)
